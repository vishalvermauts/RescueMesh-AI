    ...
    if (permissionsGranted && BleManager) {
      ...
      const runScanner = () => {
        ...
        managerRef.current.startDeviceScan(null, { allowDuplicates: true }, (error: any, device: any) => {
          ...
          if (hasOurUUID || hasOurUUIDRaw || isOurName || isOurLocalName || isMeshPattern || isAuthorizedName || isAuthorizedId) {
            console.warn(`[MESH NODE DETECTED] ${nameStr} (${device.id}) hasUUID=${hasOurUUID} hasUUIDRaw=${hasOurUUIDRaw} isMeshPattern=${isMeshPattern} nameToCheck=${nameToCheck}`);
