...
function signalColor(rssi: number | undefined) {
  ...
  // Android BLE TX power varies; -70 to -75 is often the strongest you'll see even when touching.
  if (rssi > -75) return '#10B981'; // strong: green (approx 0-5 meters)
  if (rssi > -88) return '#F59E0B'; // medium: amber (approx 5-30 meters)
  return '#EF4444';                 // weak: red (> 30 meters or blocked)
}

export default function MapScreen() {
  const { nodes, myLocation, username, permissionsGranted } = useSharedMesh();
  ...
  // Once map is ready, fit to show all active peers
  useEffect(() => {
    if (!mapReady) return;
    ...
    // Fallback to trail if no active users exist
    if (allPoints.length === 0) {
      allPoints.push(...TRAIL_COORDS);
    }

    if (allPoints.length > 0) {
      // Fix: Wait a short moment for MapView layout to finish on Android before fitting bounds
      setTimeout(() => {
  ...
  const fitAll = () => {
    ...
    if (allPoints.length === 0) {
      allPoints.push(...TRAIL_COORDS);
    } else if (allPoints.length === 1 && nodes.length === 0 && myLocation) {
      // If ONLY my location is known, and no peers are active, still show the trail as context
      // But only if I am reasonably close to the trail. 
      // A quick check: If distance from center > 50 degrees, skip trail
      if (Math.abs(myLocation[1] - TRAIL_CENTER.latitude) < 10) {
        allPoints.push(...TRAIL_COORDS);
      }
    }
  ...
  return (
    <View style={styles.container}>
      <MapView
        ...
        mapType="standard"
        showsUserLocation={permissionsGranted}
